...
#ifndef XDP_ADD_IN_LINK
#define XDP_ADD_IN_LINK     10
#endif
...
static bool multi_queue_xsks;
static bool opt_in_link;
...
struct xsk_umem_info {
	struct xsk_ring_prod fq_tmp;
	struct xsk_ring_cons cq_tmp;
...
static void print_benchmark(bool running, int sock_idx)
{
	...
	printf("%s:%d %s ", opt_if, xsks[sock_idx]->queue, bench_str);
...
static struct xsk_umem_info *xsk_configure_umem(void *buffer, u64 size)
{
	...
	ret = xsk_umem__create(&umem->umem, buffer, size, &umem->fq_tmp, &umem->cq_tmp,
			       &cfg);
	if (ret)
		exit_with_error(-ret);
...
static void xsk_populate_fill_ring(struct xsk_socket_info *xsk, int sock_idx)
{
	int ret, i;
	u32 idx;

	int fill_count = XSK_RING_PROD__DEFAULT_NUM_DESCS * 2;
	int start_frame;

	if (!multi_queue_xsks && sock_idx > 0)
		return;

    start_frame = sock_idx * fill_count;

	ret = xsk_ring_prod__reserve(&xsk->fq, fill_count, &idx);
	if (ret != fill_count)
		exit_with_error(-ret);

	for (i = start_frame; i < start_frame + fill_count; i++)
		*xsk_ring_prod__fill_addr(&xsk->fq, idx++) = i * opt_xsk_frame_size;
    
    xsk_ring_prod__submit(&xsk->fq, fill_count);
}
						    ...
						    bool rx, bool tx)
{
    ...
    if (num_socks > 0 && multi_queue_xsks) {
		xsk->queue += num_socks;
		ret = xsk_socket__create_shared(&xsk->xsk, opt_if, xsk->queue, umem->umem, 
										rxr, txr, &xsk->fq, &xsk->cq, &cfg);
	} else {
		xsk->fq = umem->fq_tmp;
		xsk->cq = umem->cq_tmp;
		ret = xsk_socket__create(&xsk->xsk, opt_if, xsk->queue, umem->umem, 
								 rxr, txr, &cfg);
	}
...
static struct option long_options[] = {
    ...
    {"chain-umem", required_argument, 0, 'Z'},
	{"in-link", no_argument, 0, 'X'},
...
static void usage(const char *prog)
{
	const char *str =
        ...
        "  -Z, --chain-umem 	Enable XDP_SHARED_UMEM for multiple queues (cannot be used with -R)"
		"  -X, --in-link		Enable in-kernel packet redirection at ingress \n"
...
static void parse_command_line(int argc, char **argv)
{
	...
	for (;;) {
		c = getopt_long(argc, argv,
				"rtli:q:pSNn:w:O:czf:muMZ:Xd:b:C:s:P:VJ:K:G:H:T:yW:U:xQaI:BRF",
    ...
    if (opt_num_xsks & (opt_num_xsks - 1)) {
        fprintf(stderr, "ERROR: --shared-umem count %d is not a power of two\n",
                opt_num_xsks);
        usage(basename(argv[0]));
    }
...
static void apply_setsockopt(struct xsk_socket_info *xsk, struct xsk_socket_info *prev_xsk)
{
	...
	if (opt_busy_poll) {
        ...
        if (setsockopt(fd, SOL_SOCKET, SO_BUSY_POLL_BUDGET,
                   (void *)&sock_opt, sizeof(sock_opt)) < 0)
            exit_with_error(errno);
    }

    if (opt_in_link && prev_xsk) {
        int prev_fd = xsk_socket__fd(prev_xsk->xsk);
        
        if (setsockopt(fd, SOL_XDP, XDP_ADD_IN_LINK,
                       &prev_fd, sizeof(prev_fd)) < 0)
            exit_with_error(errno);
    }
}
...
int main(int argc, char **argv)
{
    ...
    if (!multi_queue_xsks)
        umem_sz = (size_t)NUM_FRAMES * (size_t)opt_xsk_frame_size;
    else umem_sz = (size_t)NUM_FRAMES * (size_t)opt_xsk_frame_size * opt_num_xsks;
	...
	/* Create sockets... */
	umem = xsk_configure_umem(bufs, umem_sz);
	...
	for (i = 0; i < opt_num_xsks; i++) {
		struct xsk_socket_info *xsk = xsk_configure_socket(umem, rx, tx);
		xsks[num_socks] = xsk;

    	if (rx)
        	xsk_populate_fill_ring(xsk, num_socks);
    ...
    for (i = 0; i < num_socks; i++) {
        ...
        for (j = 0; j < num_socks; j++) {
            if (i == j)
                continue;
            apply_setsockopt(xsks[i], xsks[j]);
        }
    }
